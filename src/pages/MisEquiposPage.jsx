...
export default function MisEquiposPage() {
  ...
  // Combinar todos los equipos en una sola lista uniforme
  const todosLosEquipos = []
  if (equipoPrincipal) {
    todosLosEquipos.push(equipoPrincipal)
  }
  equipos.forEach((eq) => {
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {todosLosEquipos.map((eq) => {
              ...
              return (
                <div
                  key={eq.id_equipo}
                  className="flex flex-col justify-between rounded-[28px] bg-white p-6 shadow-sm transition-all duration-200 hover:shadow-md border border-[#EAE6FF]"
                >
                  <div>
                    {/* Header de la tarjeta */}
                    <div className="mb-5 flex items-center justify-between">
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-[#F5F3FF] px-3 py-1 text-xs font-semibold text-[#6D5BD0]">
